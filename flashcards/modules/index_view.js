...
function update_view(current_selection) {
  ...
  else if (current_selection.view == "category_form") show_category_form_view();
  else if (current_selection.view == "category_infos")
    show_category_infos_view(current_selection.category);
  else if (current_selection.view == "card_form") show_card_form_view();
...
function show_menu_view() {
  ...
  $(".menu")
    .children(".categories_amount")
    .html(Object.keys(stats).length - 1 + " categories..");
  $(".menu")
    .children(".cards_amount")
    .html(stats.all.cards + " cards..");
  $(".menu")
    .children(".cards_ready_amount")
    .html(stats.all.cards_ready + " cards ready..");
...
function show_category_form_view() {
  ...
  $(".controls")
    .children(".check_button")
    .addClass("create_category");
  $(".controls")
    .children(".check_button, .out_button")
    .css("display", "flex");
}
...
function show_category_infos_view(category) {
  ...
  if (category.name != "all") {
    $(".controls")
      .children(".trash_button, .check_button")
      .css("display", "flex");
    $(".category_infos")
      .children(".cards_ready_amount")
      .html(stats[category.name].cards_ready + " cards ready..");
    $(".category_infos")
      .children(".cards_amount")
      .html(stats[category.name].cards + " cards..");
...
function show_card_infos_view() {
  ...
  $(".controls")
    .children(".check_button")
    .addClass("update_card");
  $(".controls")
    .children(".trash_button, .check_button, .out_button")
    .css("display", "flex");
}
...
function get_categories_view() {
  let card_all = {
    ...
    content: [
      {
        tag: "h4",
        class: ["card_title", "text", "medium_text", "center", "shadow_hover"],
        text: "all"
      }
    ]
  };
  let card_add = {
    class: ["show_category_form"],
    content: [
      {
        tag: "h4",
        class: ["card_title", "text", "huge_text", "center", "shadow_hover"],
        text: "+"
      }
    ]
  };
  ...
  if (categories === null) return [card_add];
...
function get_flashcards_view(category) {
  ...
  if (flashcards === null) return category.name == "all" ? [] : [card_add];
  ...
  if (category.name != "all") view.push(card_add);
  return view;
}
