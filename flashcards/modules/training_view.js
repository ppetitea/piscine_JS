...
function show_steps_view() {
  ...
  for (const [id, flashcard] of Object.entries(flashcards))
    $(".steps_view").append(
      "<div class='step_wrapper'><div id='step" +
        id +
        "' class='step'></div></div>"
    );
}
...
function get_flashcard_form_view(flashcard) {
  ...
  if (flashcard.type == "string")
    html_form =
      "<textarea class='answer string' placeholder='string expected' spellcheck=false></textarea>";
  if (flashcard.type == "word")
    html_form =
      "<textarea class='answer word' placeholder='word expected' maxlength='26' spellcheck=false></textarea>";
  if (flashcard.type == "code")
    html_form =
      "<textarea class='answer code' placeholder='code expected' spellcheck=false></textarea>";
  if (flashcard.type == "bool")
    html_form =
      "<button class='answer true'></button><button class='answer false'></button>";
...
function get_list_form_view(answers_id) {
  ...
  for (const id of answers_id)
    html_form +=
      "<input id='answer" +
      id +
      "' class='answer list' type='text' placeholder='item" +
      id +
      "' spellcheck=false>";
...
function get_form_answer_result(flashcard) {
  ...
  if (flashcard.type == "bool")
    return get_result(
      flashcard,
      $(".true").hasClass("checked") ? "true" : "false"
    );
  if (flashcard.type == "list")
    return get_result(
      flashcard,
      get_list_form_answer(flashcard.answer.split(",").keys())
    );
}
...
function update_flashcard_remaining_days(flashcard, result) {
  ...
  if (result == false) {
    ...
    if (flashcard.repetition_delay == "6month") {
  ...
  for (const [index, card] of Object.entries(flashcards))
    if (flashcards[index].id == flashcard.id) flashcards[index] = flashcard;
